...
- **只使用** k-data-new 返回的官方 `avatar_url` 外链。管理台若展示头像图片，**只能** `<img src="avatar_url">`（或等价热链）。
- **禁止** 下载、镜像、CDN 缓存、本地存储图片二进制；**没有** 上传或静态资源管线给艺人照片。
- `source_url`、`fetched_at` 等文本字段原样来自 API，本仓库不发明抓取任务、不直连数据库。
- 非 `http(s)` 的 `data:` / `blob:` 地址一律丢弃，避免把二进制嵌进前端。
